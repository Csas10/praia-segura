...
O servidor de produção escuta em `0.0.0.0` e usa a porta definida em `PORT`
(padrão: `3000`). O endpoint `GET /api/health` pode ser usado pelo balanceador
de carga para verificar a saúde da aplicação.
...
Depois, acesse `http://localhost:3000`. O contêiner inclui um health check e
